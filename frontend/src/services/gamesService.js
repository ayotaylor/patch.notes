...
export const gamesService = {
  ...
  async removeFromLikes(userId, gameId) {
    try {
      if (
        !userId ||
      ...
      throw new Error(
        error.response?.data?.message || "Failed to remove from likes"
      );
    }
  },

  async addToLikes(userId, gameId) {
    try {
      if (
        !userId ||
        (typeof userId !== "string" && typeof userId !== "number")
      ...
      const response = await apiClient.post(`/social/likes`, body);
      return response.data.data;
    } catch (error) {
      throw new Error(
        error.response?.data?.message || "Failed to add to likes"
      );
    }
  },
  ...
  async getUserLikes(userId) {
    try {
      ...
      const response = await apiClient.get(`/social/likes/${userId}`);
      return response.data.data;
    } catch (error) {
      throw new Error(
        error.response?.data?.message || "Failed to get user's likes"
      );
    }
  },
