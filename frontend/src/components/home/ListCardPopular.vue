<template>
  <div
    class="list-card-popular cursor-pointer transition-colors duration-200"
  ...
  >
    ...
    <div class="flex flex-col gap-2">
      <!-- List Title -->
      <h4 class="font-newsreader text-lg font-bold text-theme-text-primary dark:text-theme-text-primary-dark line-clamp-2 hover:underline transition-colors duration-200">
        {{ list.title }}
      </h4>
      ...
      <div class="flex items-center justify-between text-sm">
        <div class="flex items-center gap-2">
          ...
          />
          <span class="font-tinos text-theme-text-secondary dark:text-theme-text-secondary-dark transition-colors duration-200">
            {{ list.user?.displayName || list.user?.username || 'Unknown' }}
          </span>

          <!-- Game count badge -->
          <span class="bg-gray-200 dark:bg-gray-700 text-theme-text-secondary dark:text-theme-text-secondary-dark px-2 py-0.5 rounded text-xs font-tinos transition-colors duration-200">
            {{ list.gameCount || 0 }} {{ list.gameCount === 1 ? 'game' : 'games' }}
          </span>
        </div>

        <!-- Like count -->
        <div class="flex items-center gap-1 text-theme-text-secondary dark:text-theme-text-secondary-dark transition-colors duration-200">
          <svg class="w-4 h-5" viewBox="0 0 17 21" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
            <path
